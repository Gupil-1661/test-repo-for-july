    ...
    */
    ...
    @IBAction func unwindToEmojiTVC(segue: UIStoryboardSegue) {
        ...
        guard let selectedIndexPath = tableView.indexPathForSelectedRow else {
            emojis.append(emoji)
            Emoji.saveToFile(emojis)
            let inedxPath = IndexPath(row: emojis.count - 1, section: 0)
            tableView.insertRows(at: [inedxPath], with: .fade)
            return
        }
        
        emojis[selectedIndexPath.row] = emoji
        Emoji.saveToFile(emojis)
        tableView.reloadRows(at: [selectedIndexPath], with: .fade)   // updates single row at a time
//        tableView.reloadData()    // updates multiple rows at a time
    }
